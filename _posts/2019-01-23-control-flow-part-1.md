...
- programming
---
In this series of posts, we are going to talk about the control flow in Python programming. A program’s control flow is the order in which the program’s code executes. In this first post of the series, we'll talk about
...
Most of these codes we have not seen them yet, but I will be referring to them as a statement.
...
Bear with me, I know we have not seen the for loop, we'll go in-depth later but consider a loop as a way to repeat the same code multiple time without rewriting the code.

We are using the range function that counts starting from 1 to 12. for each round, the count is increment by one and stored in the variable i. if you are keen, you may have noticed that the count started from 1 but stopped at 11 instead of 12. You may ask why is it like this? Well, this is because the range is exclusive, so 12 will not be considered; instead, the count will limit to 11.

Now let's talk about the indentation, we can see that there is a tab ( or 4 space character ) just before the print function this means that it is part of the for loop statement. Each code on the same level of indentation is part of the same block code. In Python, we delimit a block code with spaces or tab and to add more code to this block; we press ENTER.
...
In this example, we have added a print function on the last line which is not part of the for loop statement and will only be executed at last after the for loop has finished running.
...
In the example, we are having IndentationError. This error is due to the fact that we have informed Python
...
is not part of the for loop block code. To fix this error, we have to add a tab or 4 spaces in front of the print function.
...
NB: Do not mix up spaces and tab. The reason for this is that sometimes you can get into nasty bugs(errors) that are very hard to debug(to fix), so my advice is either go once for all with 4 spaces or a tab.

### Conditional Statement 

#### if and else statement

Everything we have seen so far has consisted of sequential execution, which means that codes are always executed one after the next, in precisely the order specified. But the world is often more complicated than that. 
...
* "expression" is an expression evaluated in a Boolean context, as discussed in the section on Logical Operators in the Operators and Expressions in Python tutorial.
...
Nothing surprising here the fact that we are using the format() function inside the input function which is very much possible since the format function a string's function.
...
now let's do the real thing since age cast to an integer data type, we can compare it with another integer.


```python
if age >= 21:
    print("{}, you are a grown up adult now".format(name)) #print 1
else:
    print("{}, you are still a young boy".format(name)) #print 2
```

    stern, you are a grown up adult now
...
```python
if age >= 21:
    print("{}, you are a grown up adult now".format(name))
```
...
In this example the age that we have input is 12, python will check if 12 >= 16 which is False, this means that the block code (#print 1) indented right after the if statement will not be executed at all. The execution stack (the order each code is executed in a program) will jump up to the else, and internally check if 12 >= 16, this will return True, the bock of code (#print 2)inside the else statement will be executed.

The first placeholder will display the name and the second will display the remaining age to obtain the driving license by subtracting 16 (which is the minimum age to obtain the driving license) to the current age. If the age inputted would have been higher than 16, then the first condition would have been evaluated, and the block code inside it would have been executed, the else condition with its block code will be ignored.

We have a third print (#print 3) at the end of the if-else statement, this print function will execute and printed after the if-else statement has finished its execution.
...
To demonstrate the usage of the elif statement, we are going to create a small guessing number program.
...
lastly, if none of the two conditions( if and elif statement ) is evaluated to True, this means that we are only left with the condition that your_number is equal to 5, which is the number we are guessing this means that we have won the game.
...
To be clear and explicit about what is happening, we can rewrite the expression using parenthesis, and the result should be the same. Again, the parenthesis is unnecessary; it used to make the expression more comfortable to read.
...
If any of two expressions ( age < 23 ) ( age > 85 ) result in True, the whole statement will be True because of the "or" logical operator, if any of the operand being evaluated is True then the result will be True, if both are True then the result will be True also. But if both are False only then the result will be False.
...
now let's see with an empty string and zero numerical
...
Another thing we can do is to assign the value to a variable directly.
...
Running this code, nothing happens, why? Because the if statement was executed, but after reaching the indented block code, python read the pass keyword as "just pass this line of code but do not raise an error". In these terms, pass keyword will act as a placeholder.

### Challenge 1

Challenge time!!! For your first challenge, we are going to use the challenge from the previous blog post and add to it some useful information.

The rules of this challenge are:
   * copy the codes from the previous challenge
...
Now go away and attempt these 2 challenges on your own, ONLY after you have finished or tried them you should come back and compare your solutions with mine.

### Solution to Challenge 1

Since we have already calculated the BMI, let's copy and paste the solution from the last post.
...
if (age >= 18) and (age <= 25):
    if not(gpa <= 3.00):
        print("Congratulation {}, from the fact that you have a GPA of {:5.2f} and you are {} years old, you have received a scholarship".format(name,gpa,age))
    else:
        print("Unfortunately {}, even though you are in the right age range but with a GPA of {:5.2f} we are in the incapacity of giving you a scholarship".format(name,gpa))
else:
    if not(gpa >= 3.00):
...
```
    ...
    Congratulation george,from the fact that you have a GPA of  3.66 and you are 22 years old, you have received a scholarship
...
Find the jupyter notebook version of this post on my GitHub profile [here.](https://github.com/semasuka/blog/blob/gh-pages/ipynb/Control%20Flow%20in%20Python%20Part%201%20-%20conditional%20statement%20and%20pass%20statement.ipynb){:target="_blank"}
